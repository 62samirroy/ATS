...
import CandidateSetting from '../models/CandidateSetting';

export const getSettings = async (req: Request, res: Response): Promise<void> => {
  try {
    const type = req.query.type as string;
    const filter: any = type ? { type } : {};
    const settings = await CandidateSetting.find(filter);
    res.json(settings);
  } catch (error: any) {
...
export const createSetting = async (req: Request, res: Response): Promise<void> => {
  try {
    const setting = await CandidateSetting.create(req.body);
...
export const updateSetting = async (req: Request, res: Response): Promise<void> => {
  try {
    const setting = await CandidateSetting.findByIdAndUpdate(req.params.id, req.body, { new: true });
...
export const deleteSetting = async (req: Request, res: Response): Promise<void> => {
  try {
    await CandidateSetting.findByIdAndDelete(req.params.id);
