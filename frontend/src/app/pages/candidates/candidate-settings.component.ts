  ...
  template: `
    <div class="animate-fade-in relative">
      <div class="page-header" style="display:flex;align-items:center;justify-content:space-between">
        <div>
          <h1 class="page-title"><i class="fa-solid fa-gear" style="color:var(--primary);margin-right:8px"></i> Candidate Settings</h1>
          <p class="page-subtitle">Configure candidate statuses and sources</p>
        </div>
      </div>
      ...
      <div class="card" style="padding:0;overflow:hidden;min-height:500px">
        ...
        <div style="display:flex;border-bottom:1px solid var(--border);padding:0 1rem;overflow-x:auto" class="no-scrollbar">
          <button *ngFor="let tab of tabs" (click)="activeTab=tab.id; loadSettings()"
        ...
        <div style="padding:1.5rem">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem">
            <h2 class="text-lg font-bold text-brand-text">{{ activeTab === 'status' ? 'Status List' : 'Source List' }}</h2>
            <button class="btn-primary btn-sm" (click)="openModal()">
              <i class="fa-solid fa-plus"></i> Add New {{ activeTab === 'status' ? 'Status' : 'Source' }}
            </button>
          </div>
          ...
          <div *ngIf="activeTab==='source'">
            <div *ngIf="!loading && getSources().length===0" class="text-center py-8 text-brand-muted">No sources configured.</div>
            <div *ngIf="!loading && getSources().length>0" class="space-y-3">
              <div *ngFor="let item of getSources()" class="flex items-center justify-between p-4 bg-brand-bg border border-brand-border rounded-lg">
                <div class="flex items-center gap-3">
                  <div class="w-4 h-4 rounded-full" [style.background]="item.color || '#CBD5E1'"></div>
                  <span class="font-semibold text-brand-text">{{ item.name }}</span>
                </div>
    ...
    <div *ngIf="showModal" class="modal-overlay" (click)="showModal=false">
      <div class="modal-box" (click)="$event.stopPropagation()">
        <h3 class="text-lg font-bold mb-4">{{ isEdit ? 'Edit' : 'Add' }} {{ activeTab === 'status' ? 'Status' : 'Source' }}</h3>
        
        <div class="space-y-4">
          <div class="form-group">
            <label class="form-label">Name *</label>
            <input type="text" class="form-input" [(ngModel)]="currentSetting.name" 
                   [placeholder]="activeTab === 'status' ? 'e.g. Active, Passive, Do Not Contact...' : 'e.g. LinkedIn, Referral, Indeed, Agency...'">
          </div>
    ...
    <div *ngIf="showDeleteModal" class="modal-overlay" (click)="showDeleteModal=false">
      <div class="modal-box" (click)="$event.stopPropagation()">
        ...
        <p class="text-brand-muted mb-4">Are you sure you want to delete '{{ settingToDelete?.name }}'? This might affect candidates currently using this {{ activeTab }}.</p>
...
export class CandidateSettingsComponent implements OnInit {
  activeTab: 'status' | 'source' = 'status';

  tabs = [
    { id: 'status', label: 'Candidate Statuses', icon: 'fa-solid fa-list-check' },
    { id: 'source', label: 'Candidate Sources', icon: 'fa-solid fa-share-nodes' }
  ] as const;

  settings: CandidateSetting[] = [];
  loading = true;

  showModal = false;
  isEdit = false;
  currentSetting: Partial<CandidateSetting> = { name: '', color: '#3B82F6' };

  showDeleteModal = false;
  settingToDelete: CandidateSetting | null = null;

  constructor(private candidateSettingsService: CandidateSettingsService) {}
  ...
  loadSettings() {
    this.loading = true;
    this.candidateSettingsService.getSettings().subscribe({
      next: (res) => {
        this.settings = res;
        this.loading = false;
      },
  ...
  getSources() { return this.settings.filter(s => s.type === 'source'); }

  openModal(item?: CandidateSetting) {
    if (item) {
      this.isEdit = true;
  ...
  saveSetting() {
    ...
    if (this.isEdit && this.currentSetting._id) {
      this.candidateSettingsService.updateSetting(this.currentSetting._id, this.currentSetting).subscribe(() => {
        this.showModal = false;
        this.loadSettings();
      });
    } else {
      this.currentSetting.type = this.activeTab;
      this.candidateSettingsService.createSetting(this.currentSetting as CandidateSetting).subscribe(() => {
        this.showModal = false;
        this.loadSettings();
      });
    }
  ...
  confirmDelete(item: CandidateSetting) {
  ...
  executeDelete() {
    if (this.settingToDelete && this.settingToDelete._id) {
      this.candidateSettingsService.deleteSetting(this.settingToDelete._id).subscribe(() => {
